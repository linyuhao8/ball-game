...
function drawFloor() {
    ...
    //怎麼讓ball可以回彈
    //偵測出發兩邊回彈，還是上下回彈
    let ballIsSide = false;
    loadBallSide();
    function loadBallSide() {
        // 左側
        if (ball_x + radius <= floor_x && // 球的右邊在地板的左邊
        ...
        // 右側
        if (ball_x - radius >= floor_x + floorWidth && // 球的左邊在地板的右邊
    ...
    if (ballIsSide) {
        // 左邊界碰撞
        if (ball_x + radius >= floor_x && // 球的右邊超過地板左邊
        ...
        // 右邊界碰撞
        else if (ball_x - radius <= floor_x + floorWidth && // 球的左邊超過地板右邊
